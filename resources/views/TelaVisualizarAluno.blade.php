        ...
        <table class="table table-bordered table-auto">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-teal-100">
                    @if ($usuario->usuario_id == $aluno->usuario_id)
                    <td class="text-left px-4 py-2">{{ $aluno->aluno_nome }}</td>
                    {{-- <td class="text-left px-4 py-2">{{ $professor->disciplina_id }}</td> --}}
                    <td class="text-left px-4 py-2">{{ $usuario->usuario_nome }}</td>
                    <td class="flex">
                        <a href="{{ route('editar.aluno', ['aluno_id' => $aluno->aluno_id]) }}" class="btn btn-primary btn-block">Atualizar</a>
                        <form id="delete-form-{{$aluno->aluno_id}}" action="{{ route('deletar.aluno', ['aluno_id' => $aluno->aluno_id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-block" onclick="return confirmDelete(event, {{$aluno->aluno_id}})">Deletar</button>
                        </form>
                    </td>
                    @endif
                </tr>
